...
const ModalEbast = (props) => {
    ...
    const [ebastContent, setEbastContent] = useState(
        items.map((item) => ({
            ...
            amountGoodCondition: eBast.history.length > 0 ? item.amountGoodConditionInputed : 0, //eBast.history.length > 0 ? item.amountGoodConditionInputed : item.amountGoodCondition,
            amountBadCondition: eBast.history.length > 0 ? item.amountBadConditionInputed : 0, // eBast.history.length > 0 ? item.amountBadConditionInputed : item.amountBadCondition,
            qty: item.qty,
            note: item.note ?? "",
    ...
    const doHandleChange = (e, orderProductId) => {
        ...
        if (name === "amountGoodCondition") {
            ...
            if (valNumber > -1 && Number(findData.qty) >= valNumber + Number(findData.amountBadCondition)) {
                ...
                // alert(JSON.stringify(productsUpdated))
                ...
                // console.log("assignProductsUpdate"+JSON.stringify(assignProductsUpdate))
                //   console.log("productsUpdated"+JSON.stringify(findData))
                // console.log("valNumber: "+valNumber+"-"+istotalQtyTerima+"-"+istotalBarang);
                // console.log("findDataGood"+Number(tambah)+'=='+Number(findData.qty)+'--'+Number(findData.amountGoodCondition)+'--'+Number(findData.amountBadCondition))

                // const dataxx = {amountGoodCondition:Number(tambah), orderProductId: findData.orderProductId, orderId: eBast.bastID  };
                // console.log("productsUpdated"+JSON.stringify(dataxx))

                updateEbast("qtybaik", Number(tambah), findData.orderProductId, eBast.bastID);
            } else {
            ...
            if (valNumber > -1 && Number(findData.qty) >= valNumber + Number(findData.amountGoodCondition)) {
                const tambah = valNumber == 0 ? 1 : valNumber;
                setQtyRusak(tambah);
                ...
                //     console.log("productsUpdated"+JSON.stringify(findData))
                //     }, 3000);
                // console.log("findDataBad"+Number(findData.qty)+'--'+Number(findData.amountGoodCondition)+'--'+Number(findData.amountBadCondition))
                        ...
                        <table class="table table-responsive-sm">
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td className="border-top-none p-0">:</td>
                                    <td className="border-top-none p-0">2022</td>
                                </tr>
                            </tbody>
                        </table>
                        <div></div>
                        ...
                        <button
                            ...
                            onClick={() => {
                                if (isAllQuantitiesZero()) {
                                    Swal.fire({
                                        text: "Harap isi qty terima baik atau buruk terlebih dahulu",
                                        icon: "warning",
                                        showCancelButton: false,
                                        confirmButtonColor: "#3085d6",
                                        confirmButtonText: "OK",
